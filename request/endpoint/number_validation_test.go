...
func TestNumberValidation_Validate_Value(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := p.Validate(tt.args.value); (err != nil) != tt.wantErr {
				t.Errorf("NumberValidation.Validate() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}

func TestNumberValidation_Validate_Min_Max(t *testing.T) {
	type fields struct {
		Min *float64
		Max *float64
	}
...
func TestNumberValidation_Validate_OneOf(t *testing.T) {
	type fields struct {
		OneOf []float64
	}
	type args struct {
